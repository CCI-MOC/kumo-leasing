...
It should run on the BMI client machine.
It will talk with HIL through the network.
...
"""
def check_node_poweroff(sched_timer, shutdown_time):

    node_is_NOT_powered_off = check_node()
    if (node_is_NOT_powered_off and shutdown_time >= 0):
	shutdown_time = shutdown_time - 5
	sched_timer.enter(5, 1, check_node_poweroff, (sched_timer, shutdown_time))
"""

def release_from_project(
        hil_client, node, project, shutdown_time
        ):
    """
    ...
    time.sleep(shutdown_time * 60)

    """
    sched_timer = sched.scheduler(time.time, time.sleep)
    sched_timer.enter(60, 1, check_node_poweroff, (sched_timer, shutdown_time * 60))
    sched_timer.run()
    """

    os.remove("/var/lib/tftpboot/" + node + ".ipxe")
...
def release_nodes(
        ...
        threshold_time, hil_client, shutdown_time
        ):
...
if __name__ == "__main__":
    try:
        ...
        release_nodes(
            ...
            hil_client,
            int(config.get('hil', 'shutdown_time'))
            )
